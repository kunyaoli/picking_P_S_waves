...
NAME = 'model_10_18_structure_8'
#NAME = "discussion_1.1"
EPOCH = 98

data_applied = pd.read_csv("./data/data_applied_01.csv")
data_applied = data_applied.iloc[:,1:]
print(data_applied.shape)
net = Lenet_1D()
print(net)
if torch.cuda.is_available():
    net.to('cuda')
net.load_state_dict(torch.load("./checkpoints/%s/model/Epoch%s.pth"%(NAME,EPOCH)))

p_prediction = []
s_prediction = []
for test_data in range(data_applied.shape[0]):
    x = torch.from_numpy(data_applied.iloc[test_data,:].values).to('cuda')
    ...
    print()
p_prediction_value,s_prediction_value = invers_nrom(np.array(p_prediction), np.array(s_prediction))
data_all_result = pd.DataFrame()

data_all_result["P_pred"] = np.array(p_prediction_value)
data_all_result["S_pred"] = np.array(s_prediction_value)

data_origin = pd.read_csv("../../applied/real_roi_xy/all_data_applied_T.csv")
all_result = pd.concat([data_origin, data_all_result],axis=1)
print(all_result.shape)

all_result.to_csv("../../applied/all_result.csv", index=0)
##net =Net(600, 1200, 2)
#net = Lenet_1D()
#print(net)
#if torch.cuda.is_available():
